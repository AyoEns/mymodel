...
from nets.resnet18 import ResNet18, centernet_head_resnet18
...
def decode(hm, reg, max_objects=100, num_classes=20):
    # -----------------------------------------------------#
    ...
    # -----------------------------------------------------#
    topk_x1, topk_y1 = topk_cx - 1, topk_cy - 1
    topk_x2, topk_y2 = topk_cx + 1, topk_cy + 1
...
def centernet(input_shape, num_classes, backbone='resnet50', max_objects=100, mode="train", num_stacks=2):
    assert backbone in ['resnet50', 'hourglass', 'resnet18', 'hrnet']
    ...
    x_list = Input(shape=(5,4))
    y_list = Input(shape=(5,4))
    ...
    if backbone=='resnet50':
        ...
        y1, y3 = centernet_head(C5, num_classes)
        ...
        if mode=="train":
            ...
            #     [y1, y3, hm_input, reg_input, reg_mask_input, index_input,x_list, y_list])
            # model = Model(inputs=[image_input, hm_input, reg_input, reg_mask_input, index_input,x_list, y_list], outputs=[loss_])
            loss_ = Lambda(loss, name='centernet_loss')(
                [y1, y3, hm_input, reg_input, reg_mask_input, index_input])
            model = Model(inputs=[image_input, hm_input, reg_input, reg_mask_input, index_input],
                          outputs=[loss_])
            return model
        elif mode=="predict":
            # detections = Lambda(lambda x: decode(*x, max_objects=max_objects))([y1, y2, y3])
            detections = Lambda(lambda x: decode(*x, max_objects=max_objects))([y1, y3])
        ...
        y1, y3 = centernet_head_resnet18(C5, num_classes)
        ...
        y1, y3 = HRnet(image_input, num_classes, backbone="hrnetv2_w18")
        if mode == "train":
            loss_ = Lambda(loss, name='centernet_loss')(
                [y1, y3, hm_input, reg_input, reg_mask_input, index_input])
            model = Model(inputs=[image_input, hm_input, reg_input, reg_mask_input, index_input],
